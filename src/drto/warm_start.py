...
from pyomo.core import Block, Var
...
def _block_coords(B, pos, subs):
    """The family's non-time coordinates, one per member group.

    A ``Block(time)`` family has the single empty coordinate. A family
    carrying further indexes, a ``Block(time, s)``, has one per value of
    those indexes, and each is its own walk over the time axis.
    """
    return sorted({_split_index(idx, pos, len(subs))[0] for idx in B})


def _block_times(B, pos, subs, bo):
    """The time values the family carries at the coordinate ``bo``."""
    return [
        _split_index(idx, pos, len(subs))[1]
        for idx in B
        if _split_index(idx, pos, len(subs))[0] == bo
    ]
...
def _axis(comp, time):
    ...
    # a numeric axis of its own (a cvp move set, a stage-cost set), where
    # the last index element is the point
...
def warm_start_dynamic(m):
    ...
    if recs:
        for r in recs:
            ...
            if r["kind"] == "control":
                ...
                if live is not r["of"]:
                    # cvp replaced the component, and the original reference's
        ...
        for r in recs:
            if r["kind"] == "member_subset":
                ...
                # one record per non-time coordinate of the family, so the
                # members of a Block(time, s) family reach their own copy
                bo = r.get("at", ())
                bpos, bsubs = _time_index(B, time)
                for cidx in copy:
                    o = cidx[:-1] if isinstance(cidx, tuple) else ()
                    for t in sorted(_block_times(B, bpos, bsubs, bo)):
                        c = getattr(B[_join_index(bo, t, bpos)], lname)
    ...
    # time-indexed Block families (the IDAES property idiom). Each member
    # shifts over the family's own time axis, the tail through the
    # recorded copies, algebra keeping its value past an end. A family
    # carrying further indexes, a Block(time, s), is one such walk per
    # non-time coordinate
    for B in m.component_objects(Block, active=True):
        bpos, bsubs = _time_index(B, time)
        if bpos is None:
            continue
        for bo in _block_coords(B, bpos, bsubs):
            bt = sorted(_block_times(B, bpos, bsubs, bo))
            first = B[_join_index(bo, bt[0], bpos)]
            for v in first.component_objects(Var):
                lname = v.local_name
                combos = (
                    [()]
                    if not v.is_indexed()
    ...
    # the derivatives. Each takes the previous slope one step later, and
    # the tail supplies dz/dt = gamma*(1 - tau^2)*dz/dtau past the
    # horizon's end
    ...
    # the tail itself. Every Var copy takes the trajectory at its own time
    # plus dt, still on the tail, derivatives rescale by the chain rule
    # through the map, and the pin slacks keep their values
    if recs:
        for r in recs:
            copy = r.get("copy")
