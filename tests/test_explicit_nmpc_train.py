...
# ----------------------------------------------------------------------
# the Huber gradient term
# ----------------------------------------------------------------------
def _spiked(k):
    """The toy dataset with one point's derivative labels scaled by k."""
    import copy

    d = toy_dataset(n=32)
    d.points = copy.deepcopy(d.points)
    d.points[0]["du0_dx"] = {
        uk: {xk: v * k for xk, v in row.items()}
        for uk, row in d.points[0]["du0_dx"].items()
    }
    return d


def _first_loss(k, huber):
    """The loss at the first checkpoint, the weights barely moved."""
    policy = drto.explicit_nmpc_train(
        _spiked(k),
        epochs=10,
        hidden=(8,),
    ...
    return policy.history["train_loss"][0]


def test_huber_bounds_one_sample_s_share_of_the_gradient_term():
    # doubling one sample's derivative labels quadruples a squared term,
    # and the Huber form grows with the residual rather than its square
    assert _first_loss(400, False) / _first_loss(200, False) > 3.5
    assert _first_loss(400, True) / _first_loss(200, True) < 2.5
    assert _first_loss(200, False) > 100 * _first_loss(200, True)


def test_the_huber_threshold_is_positive():
    with pytest.raises(ValueError, match="threshold is positive"):
        quick(huber_delta=0.0)


def test_below_the_threshold_huber_weighs_as_the_square_did():
    # every residual under the threshold, so gamma means the same either way
    squared = drto.explicit_nmpc_train(
        toy_dataset(),
        epochs=10,
        hidden=(8,),
        lr=1e-12,
    ...
    huber = drto.explicit_nmpc_train(
        toy_dataset(),
        epochs=10,
        hidden=(8,),
        lr=1e-12,
    ...
    assert huber.history["train_loss"][0] == pytest.approx(
        squared.history["train_loss"][0], rel=1e-12
    )
