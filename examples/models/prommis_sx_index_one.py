...
def scale(model):
    ...
    The same rules as ``prommis_sx.scale``: one factor per Var per
    species from the group's largest value, groups below the steady
    data's precision left alone, constraint factors bringing each large
    row's biggest scaled Jacobian entry to order one, rows with only
    small entries left alone, and the endpoint pins untouched.
    """
