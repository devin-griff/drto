...
| `idaes_cstr.py` | `build(N=10, h=1, ncp=3)` | The IDAES saponification CSTR, the unmodified-flowsheet example: member-subset holdup states with the water member algebraic, jacket duty and Port-declared feed flow as controls, targets and hooks as Params in the owners' units, plus the units-driven scaling (`tag_scaling`, `scaled_solve`) the notebooks and the drto initializers honor. Needs `idaes-pse`. |
