...
- With `scale` given a feature 023 source, the `scaling_factor` Suffix
  is written through `drto.scale` with that source after the first
  draw's cold start and held for every labeled solve; the default
