...
data = drto.explicit_nmpc_data(
    ...
    information=False,  # store each point's information matrix over the first moves
    solver="pounce",
    seed=0,
...
policy = drto.explicit_nmpc_train(
    ...
    weighting=None,     # None, or "information": weight both loss
                        # terms by the point's information matrix
...
before each solve and left at its last solution. With ``information``,
...
- Each labeled point carries the sampled values, the first control
  ...
  `information=True` likewise. With `information`, each labeled point
  stores its symmetric information matrix over the first moves, and
  the JSON round-trips it. A non-optimal solve is recorded as a failure and
...
- `explicit_nmpc_train` honors every listed option; `sobolev=True` on a
  ...
  a dataset without stored information matrices is a descriptive
  error.
